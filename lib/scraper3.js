...
(async () => {
  ...
  try {
    const download = await tiktok.downloader("https://vt.tiktok.com/ZSFbxWYaQ/")
    return download

    const stalk = await tiktok.stalker("superdanzyyy")
    return stalk

    const ttslide = await tiktok.slideDownloader("https://vt.tiktok.com/ZSYYYW4tk/")
    return ttslide
  } catch (error) {
    console.error(error)
    throw error
