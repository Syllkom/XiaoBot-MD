...
const bochilteam = require('@bochilteam/scraper-sosmed')
...
async function ytMp4(url, quality = '720p') {
    const data = await bochilteam.youtubedl(url);
    let desc = '';
    let views = '';
    let likes = '';
    let dislike = '';
    ...
    try {
        const video = data.video[quality];
        return {
            title: video.title,
            desc,
            views,
...
async function ytMp3(url) {
    ...
    const audioFormats = Object.values(data.audio);
    const bestAudio = audioFormats.reduce((prev, curr) => {
        return curr.bitrate > prev.bitrate ? curr : prev;
    }, audioFormats[0]);
    ...
    return {
        ...
        uploadDate,
        likes,
        result: await bestAudio.download(),
        thumb: bestAudio.thumbnail,
        quality: bestAudio.quality,
        size: bestAudio.fileSizeH
    };
}
...
module.exports = { tiktokDl, ytMp4, ytMp3, Ytdl }